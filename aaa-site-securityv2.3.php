<?php
/**
 * Plugin Name: AAA Bytelabz Security
...
if ( ! defined( 'BLZ_LOCK_ROLE_CAPS' ) )              define( 'BLZ_LOCK_ROLE_CAPS', true );
...
function blz_sec_event( $type, $detail, $alert = true ) {
	$who = function_exists( 'wp_get_current_user' ) ? wp_get_current_user() : null;
	$ip  = blz_get_secure_user_ip();

	// Throttle: a bot hitting a blocked endpoint in a loop would otherwise
	// rewrite the options row on every request. One entry per type+IP per
	// minute is enough to see what is happening.
	...
	$log = get_option( 'blz_user_events', array() );
	if ( ! is_array( $log ) ) {
		$log = array();
	}
	array_unshift( $log, $entry );
	update_option( 'blz_user_events', array_slice( $log, 0, 200 ), false );

	blz_scan_log( strtoupper( $type ) . ': ' . $detail . ' | ip=' . $entry['ip'] . ' | actor=' . $entry['user'] . ' | uri=' . $entry['uri'] );

	if ( $alert ) {
		blz_sec_alert( $type, $entry );
	}
}

function blz_sec_alert( $type, $entry ) {
	if ( get_transient( 'blz_alert_sent_' . $type ) ) {
		return; // one email per event type per 15 minutes
	}
	set_transient( 'blz_alert_sent_' . $type, 1, 15 * MINUTE_IN_SECONDS );
	...
	$body = array(
		'A user-account security event was recorded on ' . home_url() . ':',
		'',
		'Event:  ' . $type,
		'Detail: ' . $entry['detail'],
		'Actor:  ' . $entry['user'],
		...
		'',
		'Full log: ' . admin_url( 'tools.php?page=blz-user-audit' ),
		'(Further emails of this type are suppressed for 15 minutes.)',
	);

	wp_mail( $to, '[' . get_bloginfo( 'name' ) . '] User security event: ' . $type, implode( "\n", $body ) );
}
...
add_filter( 'pre_update_option_' . $blz_role_key, function ( $value, $old_value ) {
	...
	blz_sec_event( 'blocked_role_edit', 'BLOCKED unauthorised write to the role definition table', true );
	return $old_value;
}, 10, 2 );
...
if ( blz_login_slug() ) {
	...
	add_action( 'plugins_loaded', function () {
		...
		// (A) Secret slug requested -> hand off to wp-login.php.
		if ( $path === $slug ) {
		...
		// (B) Direct hit on the real wp-login.php -> 404.
...
function blz_scan_log( $message ) {
	...
	@file_put_contents( trailingslashit( $dir ) . 'scan.log', $line, FILE_APPEND | LOCK_EX );
}
...
function blz_notify_findings( $report ) {
	...
	$lines   = array();
	$lines[] = 'Automated security scan on ' . home_url() . ' found the following:';
	$lines[] = '';
	if ( ! empty( $c['error'] ) ) {
		$lines[] = 'Core check error: ' . $c['error'];
	}
	$sections = array(
		'MODIFIED core files'           => $c['modified'],
		'MISSING core files'            => $c['missing'],
		'UNKNOWN files in core dirs'    => $c['unknown'],
		'QUARANTINED uploads PHP files' => $u['quarantined'],
	);
	...
	$lines[] = 'Review: ' . admin_url( 'tools.php?page=blz-security-scan' );

	wp_mail( $to, '[' . get_bloginfo( 'name' ) . '] Security scan found issues', implode( "\n", $lines ) );
}
...
function blz_render_user_audit_page() {
	...
	if ( isset( $_POST['blz_clear_events'] ) ) {
		...
		update_option( 'blz_user_events', array(), false );
	...
	$log = get_option( 'blz_user_events', array() );
	echo '<h2>Event log (' . count( (array) $log ) . ')</h2>';
	if ( empty( $log ) ) {
		echo '<p>Nothing recorded yet.</p>';
	} else {
		echo '<table class="widefat striped"><thead><tr><th>When (UTC)</th><th>Event</th><th>Detail</th><th>Actor</th><th>IP</th><th>Request</th></tr></thead><tbody>';
		foreach ( array_slice( (array) $log, 0, 100 ) as $e ) {
			echo '<tr><td>' . esc_html( gmdate( 'Y-m-d H:i:s', (int) $e['time'] ) ) . '</td>'
				. '<td><code>' . esc_html( $e['type'] ) . '</code></td>'
				. '<td>' . esc_html( $e['detail'] ) . '</td>'
				. '<td>' . esc_html( $e['user'] ) . '</td>'
				. '<td>' . esc_html( $e['ip'] ) . '</td>'
				. '<td><code>' . esc_html( $e['uri'] ) . '</code></td></tr>';
		}
		echo '</tbody></table>';
	}
	...
	echo '<button type="submit" name="blz_clear_events" value="1" class="button">Clear event log</button> ';
